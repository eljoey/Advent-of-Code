...
let inputFormatted = formatArr(input);

const passportProcessing = (arr) => {
    let count = 0;
    for (let i = 0; i < arr.length - 1; i++) {

        if (!(arr[i]['byr'] && arr[i]['iyr'] && arr[i]['eyr'] && arr[i]['hgt'] && arr[i]['hcl'] && arr[i]['ecl'] && arr[i]['pid'])) {
            continue;
        }
        if (validateBYR(arr[i]['byr']) && validateIYR(arr[i]['iyr']) && validateEYR(arr[i]['eyr']) && validateHGT(arr[i]['hgt']) && validateHCL(arr[i]['hcl']) && validateECL(arr[i]['ecl']) && validatePID(arr[i]['pid'])) {
            count++;
        }
...
const validateBYR = (string) => {
    if (string >= 1920 && string <= 2002) {
        return true;
    }
    return false;
};

const validateIYR = (string) => {
    if (string >= 2010 & string <= 2020) {
        return true;
    }
    return false;
};

const validateEYR = (string) => {
    if (string >= 2020 && string <= 2030) {
        return true;
    }
    return false;
};

const validateHGT = (string) => {
    let inches = string[string.length - 2] === 'i' ? true : false;
    let cm = string[string.length - 2] === 'c' ? true : false;
    let value = string.substring(0, string.length - 2);

    if (!cm && !inches) {
        return false;
    }

    if (inches && (value < 59 || value > 76)) {
        return false;
    }

    if (cm && (value < 150 || value > 193)) {
        return false;
    }
    return true;
};

const validateHCL = (string) => {
    let regex = RegExp('#([0-9]|[a-f]){6}');
    return regex.test(string);
};

const validateECL = (string) => {
    let options = ['amb', 'blu', 'brn', 'gry', 'grn', 'hzl', 'oth'];
    return options.includes(string);
};

const validatePID = (string) => {
    let regex = RegExp(/^\d{9}/g);
    return regex.test(string);
};
console.log(passportProcessing(inputFormatted));